...
import { Bell, BookOpen, Bot, CalendarClock, Circle, CircleHelp, Home, Landmark, MessageSquare, Search, Settings2, ShieldAlert, SlidersHorizontal, Sparkles, Users, Vault, X } from "lucide-react";
...
const PANELS: Record<string, Item[]> = {
  ...
  Vault: [{ label: "Internal", detail: "Aetoes firm documents", icon: Vault, to: "/search" }, { label: "Juris OS", detail: "Public jurisprudence", icon: Landmark, to: "/search" }, { label: "Search", detail: "Cross-vault research", icon: Search, to: "/search" }, { label: "Access requests", detail: "Named grants and approvals", icon: ShieldAlert, to: "/access-requests" }],
...
function TaskBarItem({ icon: Icon, label, to, badge }: { icon: typeof Home; label: string; to: "/pins" | "/chats" | "/channels" | "/threads" | "/contacts"; badge?: string }) {
  return <Link to={to} className="group flex items-center gap-1.5 rounded-md px-2 py-1.5 transition-all duration-200 hover:bg-sidebar-accent hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" title={label}>
    <Icon className="size-3.5" /><span>{label}</span>{badge && <span className="rounded-full bg-sidebar-accent px-1.5 font-mono text-[9px] text-steel">{badge}</span>}
  </Link>;
}
...
export function AppShell({ title, eyebrow, children }: { title: string; eyebrow: string; children: ReactNode }) {
  ...
  return <div className="flex min-h-screen bg-background">
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      <header className="sticky top-0 z-20 border-b border-border bg-background/90 px-5 py-4 backdrop-blur-xl lg:px-8"><div className="flex items-center justify-between gap-4"><div className="min-w-0"><div className="font-mono text-[10px] uppercase tracking-[0.28em] text-steel">{eyebrow}</div><h1 className="mt-1 truncate text-2xl font-semibold lg:text-3xl">{title}</h1></div><div className="flex items-center gap-2 text-xs text-muted-foreground"><button type="button" className="hidden items-center gap-3 rounded-lg border border-border bg-surface px-3 py-2 text-left transition-all duration-200 hover:border-gold/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold sm:flex" onClick={() => setPaletteOpen(true)} aria-label="Open command palette"><Search className="size-4 text-steel" /><span className="hidden lg:inline">Search anything</span><kbd className="rounded border border-border px-1.5 py-0.5 font-mono text-[9px]">⌘K</kbd></button><button type="button" className="rounded-lg p-2 transition-all duration-200 hover:bg-surface hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" aria-label="Notifications"><Bell className="size-4" /></button><button type="button" onClick={replayChromeTour} className="rounded-lg p-2 transition-all duration-200 hover:bg-surface hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" aria-label="Help and replay guided tour"><CircleHelp className="size-4" /></button><span className="hidden rounded-full border border-gold/40 px-3 py-1 font-mono text-[10px] text-gold xl:inline">CONFIDENTIAL · PRIVILEGED</span><span className="flex items-center gap-2 rounded-full border border-border/70 bg-sidebar px-2.5 py-1"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-gold/20 text-[9px] font-bold text-gold">{identity.name.slice(0, 2).toUpperCase()}</span><span className="hidden font-medium text-foreground sm:inline">{identity.name}</span></span></div></div><nav className="mt-4 flex gap-2 overflow-x-auto lg:hidden">{visibleRail.filter((item) => item.to).map((item) => <Link key={item.label} to={item.to!} className="whitespace-nowrap rounded-md border border-border px-3 py-1.5 text-xs transition-all duration-200 hover:border-gold/50">{item.label}</Link>)}</nav></header>
      <main className={`shell-main flex-1 px-5 py-8 transition-all duration-200 lg:px-8 ${compact ? "lg:py-5" : ""}`}>{children}</main>
      <footer className="sticky bottom-0 z-10 flex items-center justify-between gap-4 overflow-x-auto border-t border-border bg-sidebar/95 px-5 py-2.5 backdrop-blur-xl lg:px-8"><nav aria-label="Communication task bar" className="flex min-w-max items-center gap-1 text-[11px] text-muted-foreground"><TaskBarItem icon={BookOpen} label="Pins" to="/pins" /><TaskBarItem icon={MessageSquare} label="Chats" to="/chats" /><TaskBarItem icon={Landmark} label="Channels" to="/channels" /><TaskBarItem icon={CircleHelp} label="Threads" to="/threads" /><TaskBarItem icon={Users} label="Contacts" to="/contacts" /></nav><div className="flex min-w-max items-center gap-2"><span className="hidden items-center gap-1.5 text-[10px] text-muted-foreground sm:flex"><Circle className="size-2 fill-success text-success" /> Team online</span><button type="button" onClick={() => setAssistantOpen(true)} className="flex items-center gap-1.5 rounded-md bg-gold/10 px-2 py-1 text-[11px] font-medium text-gold transition-all duration-200 hover:bg-gold/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" aria-label="Open Assistant dock"><Bot className="size-3.5" /> Assistant</button><button type="button" onClick={() => setCompact((value) => !value)} className="flex shrink-0 items-center gap-2 rounded-md px-2 py-1 text-[11px] text-muted-foreground transition-all duration-200 hover:bg-sidebar-accent hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" aria-label="Toggle density"><Settings2 className="size-3.5" /> {compact ? "Compact" : "Comfortable"}</button></div></footer>
    </div>
    <ChromeTour isFirmAdmin={firmAdmin} />
    <CommandPalette open={paletteOpen} onOpenChange={setPaletteOpen} onAssistant={() => setAssistantOpen(true)} onToggleDensity={() => setCompact((value) => !value)} />
