...
function TaskBarItem({ icon: Icon, label, badge }: { icon: typeof Home; label: string; badge?: string }) {
  return (
    <span className="group flex items-center gap-1.5 rounded-md px-2 py-1.5 transition-all duration-200 hover:bg-sidebar-accent hover:text-foreground" title={`${label} will be available when messaging ships`}>
      <Icon className="size-3.5" />
      <span>{label}</span>
      {badge && <span className="rounded-full bg-sidebar-accent px-1.5 font-mono text-[9px] text-steel">{badge}</span>}
...
export function AppShell({ title, eyebrow, children }: { title: string; eyebrow: string; children: ReactNode }) {
  ...
  return <div className="flex min-h-screen bg-background">
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      ...
      <footer className="sticky bottom-0 z-10 flex items-center justify-between gap-4 overflow-x-auto border-t border-border bg-sidebar/95 px-5 py-2.5 backdrop-blur-xl lg:px-8"><nav aria-label="Communication task bar" className="flex min-w-max items-center gap-1 text-[11px] text-muted-foreground"><TaskBarItem icon={BookOpen} label="Pins" /><TaskBarItem icon={MessageSquare} label="Chats" badge="0" /><TaskBarItem icon={Landmark} label="Channels" /><TaskBarItem icon={CircleHelp} label="Threads" /><TaskBarItem icon={Users} label="Contacts" /></nav><button type="button" onClick={() => setCompact((value) => !value)} className="flex shrink-0 items-center gap-2 rounded-md px-2 py-1 text-[11px] text-muted-foreground transition-all duration-200 hover:bg-sidebar-accent hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-gold" aria-label="Toggle density"><Settings2 className="size-3.5" /> {compact ? "Compact" : "Comfortable"}</button></footer>
    </div>
  </div>;
}
