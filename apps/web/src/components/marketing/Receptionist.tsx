...
export function Receptionist() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      text: "Hello — I'm RedCase's receptionist. Ask me about the platform, security, or what's shipped vs. on the roadmap.",
    },
  ]);
  const [input, setInput] = useState("");

  const last = messages[messages.length - 1];
  const showingEscalate = last.role === "assistant" && last.text === ESCALATE;
  ...
  return (
    <section id="faq" className="mx-auto max-w-3xl px-5 py-20 lg:px-8 lg:py-28">
      <Eyebrow>Talk to the receptionist</Eyebrow>
      <h2 className="mt-4 font-display text-3xl font-semibold leading-tight">
        Questions, answered plainly — or routed to a human.
      </h2>

      <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-surface/60 shadow-elevated">
        <div className="flex items-center gap-2 border-b border-border/60 bg-background/40 px-5 py-3">
          <Sparkles className="size-4 text-gold" />
          <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
            Receptionist · curated knowledge only
          </span>
        </div>

        <div ref={listRef} className="max-h-[22rem] space-y-3 overflow-y-auto px-5 py-5">
          {messages.map((msg, i) => (
            <div
              key={i}
        ...
        {showingEscalate && (
          <div className="flex items-center gap-2 border-t border-border/60 bg-amber-400/5 px-5 py-2.5 text-xs text-amber-200/90">
        ...
        <div className="border-t border-border/60 p-3">
          ...
          >
            ...
            <button
              type="submit"
              className="inline-flex h-11 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground transition-all duration-200 hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
              aria-label="Send"
            >
