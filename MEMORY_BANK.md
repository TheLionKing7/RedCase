...
- **Collaboration UI follow-up — IN PROGRESS (2026-09-24):** `apps/web/src/components/CollaborationWorkspace.tsx` is modified but not committed. The working tree contains direct-chat recipient creation, pin resource-type selection, pin/message/thread mutation loading states, and visible error states. Confirmed UTF-8 BOMs were removed from `apps/api/tests/test_persona.py`, `apps/web/src/components/AppShell.tsx`, `apps/web/src/components/CollaborationWorkspace.tsx`, `apps/web/src/routes/_authed.search.tsx`, and `apps/web/src/routes/_authed.workbench.tsx`; ordinary Unicode punctuation was preserved. Full API suite passed separately: `267 passed, 51 skipped, 9 warnings` in 157.77s. Frontend production build passed separately in 4.47s; only dependency/tooling warnings were emitted. Final tracked-file encoding scan reports no UTF-8 BOMs or high-confidence mojibake sequences, and `git diff --check` reports no whitespace errors. The collaboration UI and BOM cleanup are ready to commit; unrelated untracked artifacts remain untouched.
