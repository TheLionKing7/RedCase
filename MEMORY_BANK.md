...
- **SLICE 0 identity fallback (S10-0 continuation) — IMPLEMENTED, build verified (2026-09-24):**
  ...
  - Visual caveat: the existing `slice0-home.png` was captured from plain `vite dev` without `VITE_API_DEV_ADAPTER=1`, so it shows terminal `Counsel`/`CO`, not the intended adapter persona. Re-capture with the adapter enabled before treating the screenshot as final.
