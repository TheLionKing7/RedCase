...
- **M1 Part 1+2 — Assistant tools consolidation (IN PROGRESS, backend done):** `expert_chat.py` deprecation docstring; `ALLOWED_TOOLS` += show_overview/show_arguments/show_similar_cases/show_law (with AGENT_SYSTEM doubled-brace `.format()` entries); service.py adds `_load_analysis_sections` (queries `document_analyses WHERE id AND created_by=$2`, decodes `output.sections`) + four `_show_*` tool fns (owned analysis only, honest refusal when missing/no section — never fabrication) wired into `_run_tool` dispatch; `tests/test_assistant_tools.py` 7/7 (FakeConn returns dict `{"output": ...}` — asymmetric-record needs `row["output"]` subscript, so use dict not object). Web: `components/assistant/AnalysisSectionCard.tsx` created (still unwired to chat renderer). **NOT YET COMMITTED.**
