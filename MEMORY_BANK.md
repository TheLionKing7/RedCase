...
### In Progress / Next
- **UX-3 ✅ manually verified, ready to commit:** Home widget customization persists the four Inbox/Today/My deadlines/My matters toggles; bottom task bar exposes Pins/Chats/Channels/Threads/Contacts, Team online presence, and Assistant entry. Browser verification at `http://127.0.0.1:7100/home` confirmed Inbox disappears when toggled off and Assistant navigated to `/workbench` before UX-4 changed that entry to open the dock. Intended UX-3 files: `apps/web/src/components/AppShell.tsx`, `apps/web/src/routes/_authed.home.tsx`.
- **UX-4 ✅ implemented and browser-verified, ready to commit:** `components/ChromeTour.tsx` adds a skippable/replayable five-step first-run chrome tour keyed per user in localStorage; `components/AssistantDock.tsx` adds a right-side Assistant thread dock reusing the existing thread/SSE contract; `AppShell.tsx` mounts both, Help replays the tour, and the task-bar Assistant opens the dock. `npm run build` GREEN; `git diff --check` GREEN (only pre-existing line-ending warnings). Browser verified tour steps, Skip dismissal, Help replay affordance, Assistant empty state, and accessible close button. Existing third-party `use client` build warnings unchanged.
...
- **Runtime/database verification + collaboration follow-up — COMPLETED (2026-09-24):** provisioned `apps/api/.venv312` with the declared locked dependencies via `uv`; direct package installation was required because the pre-existing environment did not contain pip/modules and `uv sync --no-install-project` did not populate it. Live `python -m alembic heads/current/upgrade head` against `apps/api/.env` succeeded: database advanced `0020 → 0021 → 0022 → 0023 → 0024d + 0024 → 0025`, with `0025` as the sole head. Corrected collaboration contracts: `/v1/members` directory now belongs to `app/routers/members.py` and is tenant-scoped/sorted; removed its duplicate generic collaboration route; pin deletion validates UUIDs and exact affected-row count; frontend SSE parsing buffers split frames; direct-channel creation hook is exposed. Added directory isolation/order coverage. Focused live integration suite `pytest tests/test_members.py tests/test_channels.py tests/test_assistant.py -q` passes **27/27**. Unrelated untracked artifacts remain untouched.
- **Collaboration UI follow-up — COMPLETED (2026-09-24):** `apps/web/src/components/CollaborationWorkspace.tsx` includes direct-chat recipient creation, pin resource-type selection, pin/message/thread mutation loading states, and visible error states. UTF-8 BOMs were removed from the affected tracked files while ordinary Unicode punctuation was preserved. Full API suite passed separately: `267 passed, 51 skipped, 9 warnings` in 157.77s. Frontend production build passed separately in 4.47s. Final tracked-file encoding scan reports no UTF-8 BOMs or high-confidence mojibake sequences, and `git diff --check` reports no whitespace errors. Changes committed as `d987d2f`; unrelated untracked artifacts remain untouched.
- **UX-2 services + departments — IMPLEMENTED and build-verified (2026-09-24):** Onboarding step 3 presents selectable practice services and firm departments, keeps Legal Practice locked on, persists services through the existing firm practice-area endpoint and departments through `/v1/persona/departments` during finish, and retains local draft state for offline handoff. Added `useDepartments()` to the typed web client; AppShell keeps Firm Ops admin-only and additionally hides it when no optional operations department (Accounts & Finance, HR & Administration, Operations) is enabled, while defaulting visible during query loading/failure. Added department API coverage for replacement/deduplication, mandatory Legal Practice, non-admin denial, and tenant isolation in `tests/test_persona.py`. Focused API tests and `npm run build` pass; known dependency `use client` warnings remain. Commit: `c676a51`; unrelated untracked artifacts remain untouched.
- **UX-3 Home + task bar enhancements — IMPLEMENTED and build-verified (2026-09-24):** Added a native accessible “Customize home” disclosure to `apps/web/src/routes/_authed.home.tsx`, allowing users to show/hide Inbox, Today, My deadlines, and My matters widgets for the current session. Enhanced `apps/web/src/components/AppShell.tsx` bottom task bar with team presence status and an Assistant entry point linking to the existing Workbench surface. Frontend production build passed in 4.52s; `git diff --check` passed. Existing dependency `use client` warnings remain. Changes are uncommitted; unrelated untracked artifacts remain untouched.
