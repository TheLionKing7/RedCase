...
- **UX-2 services + departments — IMPLEMENTED and build-verified (2026-09-24):** Onboarding step 3 presents selectable practice services and firm departments, keeps Legal Practice locked on, persists services through the existing firm practice-area endpoint and departments through `/v1/persona/departments` during finish, and retains local draft state for offline handoff. Added `useDepartments()` to the typed web client; AppShell keeps Firm Ops admin-only and additionally hides it when no optional operations department (Accounts & Finance, HR & Administration, Operations) is enabled, while defaulting visible during query loading/failure. Added department API coverage for replacement/deduplication, mandatory Legal Practice, non-admin denial, and tenant isolation in `tests/test_persona.py`. Focused API tests and `npm run build` pass; known dependency `use client` warnings remain. Changes are uncommitted; unrelated untracked artifacts remain untouched.
